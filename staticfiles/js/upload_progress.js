document.addEventListener('DOMContentLoaded', function() {
    ...
    inlineForms.forEach(function(inline) {
        ...
        // "Add another" tugmasini kattaroq qilish va yuqoriga ko'chirish
        const addRowLink = inline.querySelector('.add-row a');
        if (addRowLink) {
            addRowLink.style.fontSize = '18px';
            addRowLink.style.fontWeight = 'bold';
            addRowLink.style.padding = '10px 20px';
            addRowLink.style.backgroundColor = '#28a745';
            addRowLink.style.color = 'white';
            addRowLink.style.borderRadius = '5px';
            addRowLink.style.textDecoration = 'none';
            addRowLink.style.display = 'inline-block';
            addRowLink.style.marginBottom = '10px';

            // Yuqoriga ko'chirish
            const inlineTable = inline.querySelector('table');
            if (inlineTable) {
                const addRowTr = addRowLink.closest('tr');
                if (addRowTr) {
                    inlineTable.insertBefore(addRowTr, inlineTable.firstChild);
                }
            }
        }
    });
